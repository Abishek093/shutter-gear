...
const generateSalesReport = (orders, res) => {
    const doc = new PDFDocument();
    doc.text('Shutter Gear', { align: 'center' });
    doc.moveDown();
    ...
    orders.forEach((order) => {
        ...
        order.products.forEach((product) => {
            doc.text(`Product: ${product.product.title}`, { fontSize: 12 });
            doc.text(`Status: ${product.status}`, { fontSize: 12 });
            doc.text(`Price: ₹${product.total}`, { fontSize: 12 });
        ...
        doc.text(`Order Status: ${order.status}`, { fontSize: 12 });
        doc.text(`Date: ${order.createdAt ? order.createdAt.toLocaleDateString() : 'N/A'}`, { fontSize: 12 });
        doc.moveDown();
    });
...
const generateInvoicePdf = (orderDetails, res) => {
    ...
    orderDetails.products.forEach((item) => {
        // Ensure that 'product' is populated in the 'item' object
        const product = item.product;

        if (product) {
            doc.text(`Product: ${product.title}`, { fontSize: 12 });
            doc.text(`Quantity: ${item.quantity}`, { fontSize: 12 });
            doc.text(`Price: ₹${item.price}`, { fontSize: 12 });
            doc.text(`Status: ${item.status}`, { fontSize: 12 });
            doc.moveDown();
        }
    });
    ...
    doc.text(`Order Total: ₹${totalPrice}`, { fontSize: 12 });
    doc.text(`Address: ${orderDetails.address.address}`, { fontSize: 12 });
    doc.text(`Payment Method: ${orderDetails.paymentMethod}`, { fontSize: 12 });
